...
class ChunkState:
    ...
    def __init__(self, key_words: list[int], chunk_counter: int, flags: int, rnd:int) -> None:
...
class Hasher:
    ...
    def _init(self, key_words: list[int], flags: int, rnd) -> None:
        assert len(key_words) == 8
        self.chunk_state = ChunkState(key_words, 0, flags, rnd)
    ...
    def update(self, input_bytes: bytes) -> None:
        while input_bytes:
            ...
            if self.chunk_state.len() == CHUNK_LEN:
                ...
                self.chunk_state = ChunkState(self.key_words, total_chunks, self.flags, self.rnd)
